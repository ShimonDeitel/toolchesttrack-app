    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showAdd) {
                AddToolView(isPresented: $showAdd)
                    .environmentObject(store)
            }
            .sheet(item: $editingItem) { existing in
                AddToolView(isPresented: Binding(get: { editingItem != nil }, set: { if !$0 { editingItem = nil } }), editing: existing)
                    .environmentObject(store)
            }
...
struct AddToolView: View {
    ...
    var editing: Tool? = nil

    @State private var draft: Tool = Tool(name: "", maker: "", condition: "", notes: "")
